...
# Los animales que no se encuentran en ambas listas son 3 dinosaurios: 
# El diplodocus, el triceratops y el brancosaurio.
...
# Ambas se encuentran dentro del rango por lo que podemos afirmar que siguen 
...
is.na(cabbages$VitC)
# Ninguna de las variables del conjunto cabbages contiene valores NA
...
sum(is.na(Chile))
# Existen 295 valores perdidos en el conjunto Chile
...
# Ambas se ajustan adecuadamente a la curva.
...
```{r}
cultivos <- levels (cabbages$Cult)

hist(cabbages[which(cabbages$Cult==cultivos[1]),]$VitC, main = "VitC in c39",
     col = "red", xlab = "Vitamina C")
hist(cabbages[which(cabbages$Cult==cultivos[2]),]$VitC, main = "VitC in c52",
     col = "cyan", xlab = "Vitamina C")

# Se puede observar como el segundo tipo de cultivo c52 tiene valores de vitamina
# C mas elevados en general
...
# Se observa una cierta correlacion pero no parece ser especialmente fuerte
...
c39 <- plot(cabbages[which(cabbages$Cult == cultivos[1]),]$VitC,
              cabbages[which(cabbages$Cult == cultivos[1]),]$HeadWt, pch = 16)
c39

c52 <- plot(cabbages[which(cabbages$Cult == cultivos[2]),]$VitC,
              cabbages[which(cabbages$Cult == cultivos[2]),]$HeadWt, pch = 15)
c52
# Se puede observar claramente como la segunda variedad tiene una mayor 
# correlacion que la primera
